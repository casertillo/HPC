    ...
    int** obstacles_ptr, float** av_vels_ptr, float** tmp_av_vels_ptr)
{
    ...
    /*ASSIGNED THE NUMBER OF TOTAL GROUPS BY DIVIDING WITH THE LOCAL SIZE*/
    //HARD CODED----------------------------------------------------------
    //--------------------------------------------------------------------
    params->num_groups = (params->nx*params->ny)/256;

    //WORK AROUND FOR GLOBAL SIZE WHEN IS LARGE PIPE
    if(params->nx == 1000)
    {
        params->global_size_x = 1024;
        params->global_size_y = 1024;
    } 
    else
    {
        params->global_size_x = params->nx;
        params->global_size_y = params->ny;
    }
    //--------------------------------------------------------------------
    //--------------------------------------------------------------------
    ...
    *tmp_av_vels_ptr = (float*) malloc(sizeof(float)*(params->max_iters*params->num_groups));
    if (*tmp_av_vels_ptr == NULL) DIE("Cannot allocate memory for tmp_av_vels");
