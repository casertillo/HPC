    ...
    speed_t* cells, int* obstacles)
{
    ...
    {
        jj = accel_area.idx;
        #pragma omp parallel for firstprivate(jj, w1, w2, cells, obstacles) schedule(static) 
    ...
    {
        ii = accel_area.idx;
	#pragma omp parallel for firstprivate(ii, w1, w2, cells, obstacles) schedule(static) 
...
void propagate(const param_t params, speed_t* cells, speed_t* tmp_cells)
{
    ...
    #pragma omp parallel for firstprivate(tmp_cells, cells) schedule(static) collapse(2)  default(none)
    for (ii = 0; ii < params.ny; ii++)
    {
        for (jj = 0; jj < params.nx; jj++)
        {
...
void rebound(const param_t params, speed_t* cells, speed_t* tmp_cells, int* obstacles)
{
    ...
    #pragma omp parallel for firstprivate(tmp_cells, obstacles, cells) schedule(static) collapse(2) default(none) 
    for (ii = 0; ii < params.ny; ii++)
    {
        for (jj = 0; jj < params.nx; jj++)
        {
...
void collision(const param_t params, speed_t* cells, speed_t* tmp_cells, int* obstacles)
{
    ...
    ** are in the scratch-space grid */
    #pragma omp parallel for private(u_x, u_y, u_sq, u, d_equ, local_density) firstprivate( obstacles, kk, cells, tmp_cells)  collapse(2)  schedule(static) default(none)  
    for (ii = 0; ii < params.ny; ii++)
    {
        for (jj = 0; jj < params.nx; jj++)
        {
		...
		*(d_equ+2) = w1 * local_density * (1.0 + *(u+2) / c_sq + (*(u+2) * *(u+2)) / (2.0 * c_sq * c_sq)- u_sq / (2.0 * c_sq));
                *(d_equ+3) = w1 * local_density * (1.0 + *(u+3) / c_sq + (*(u+3) * *(u+3)) / (2.0 * c_sq * c_sq)- u_sq / (2.0 * c_sq));
                *(d_equ+4) = w1 * local_density * (1.0 + *(u+4) / c_sq + (*(u+4) * *(u+4)) / (2.0 * c_sq * c_sq)- u_sq / (2.0 * c_sq));
		...
		/* diagonal speeds: weight w2 */
                ...
                *(d_equ+7) = w2 * local_density * (1.0 + *(u+7) / c_sq + (*(u+7) * *(u+7)) / (2.0 * c_sq * c_sq)- u_sq / (2.0 * c_sq));
                *(d_equ+8) = w2 * local_density * (1.0 + *(u+8) / c_sq + (*(u+8) * *(u+8)) / (2.0 * c_sq * c_sq)- u_sq / (2.0 * c_sq));
...
double av_velocity(const param_t params, speed_t* cells, int* obstacles)
{
    ...
    #pragma omp parallel for firstprivate(obstacles, cells, local_density, kk) private(u_x, u_y) collapse(2)  schedule(static) default(none) reduction(+:tot_cells, tot_u)
    for (ii = 0; ii < params.ny; ii++)
    {
        for (jj = 0; jj < params.nx; jj++)
        {
